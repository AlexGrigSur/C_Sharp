...
namespace NavProject_Drawing.CalcFunctions
{
    abstract class DijsktraBasic
    {
        ...
        protected abstract object mainStruct { get; set; }
        protected abstract Dictionary<object, int> nodesToBeVisited { get; set; }
        ...
        protected abstract void MainHandler();
    ...
    class DijkstraForConnectivitySplit : DijsktraBasic
    {
        ...
        public void Run(Map map)
        {
            ...
            foreach (int floorIndex in map.GetFloorsKeys())
            {
                map.ClearConnectivityComponentsOnLevel(floorIndex);
                Level currentLevel = map.GetFloor(floorIndex);
                Dictionary<object, int> nodesToBeVisitedLocal = new Dictionary<object, int>(); // 0-notVisited ,1-reachable, 2-visited

                foreach (Node j in currentLevel.GetNodeListOnFloor//GetNodeListOnFloor().Keys)
    ...
    class DijkstraForMapConnectivity : DijsktraBasic
    {
        public bool Run(Map map)
        {
            ...
            foreach (Level i in map.GetFloorsValues())//map.GetFloorsList().Values)
            {
